...
describe('API Endpoints', () => {
  ...
  test('PUT /api/accounts/password - should update password if current password matches', async () => {
    // Mock current password check
    db.query
      .mockResolvedValueOnce([[{ password: 'oldpass' }]]) // For SELECT password
      .mockResolvedValueOnce([{ affectedRows: 1 }]);       // For UPDATE password
